...
# ตัวอย่างประเมิน
        ...
        rain_condition = (prob_mueang >= 0.5) or (prob_mae_rim >= 0.5)
        # ===============================
        # 1) INPUT
        # ===============================
        ...
        S = sorted(U, key=U.get, reverse=True)[:12]
        N = min(N, len(S))
        V = [hotel_name] + S
        Type = {p["name"]: p.get("type", "outdoor") for p in S_all if p["name"] in S}
        # ===============================
        # 3) TIME / COST
        # ===============================
        ...
        Z1 = gp.quicksum(U[i] * Y[i, d] for i in S for d in D)   # rating
        Z2 = gp.quicksum(Tij[i, j] * X[i, j, d] for (i, j, d) in X.keys())  # time
        Z3 = gp.quicksum(Cij[i, j] * X[i, j, d] for (i, j, d) in X.keys())  # money
        ...
        if mode == "rating":
            ...
            objective = w2*Z2 + w1*Z1 + w3*Z3
            sense = GRB.MINIMIZE

        elif mode == "money":
            w1, w2, w3 = 0.4, 0.2, 0.4
            objective = w3*Z3 + w1*Z1 + w2*Z2
            sense = GRB.MINIMIZE

        m.setObjective(objective, sense)
